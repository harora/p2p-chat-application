...
class ChatClient(Frame):
  ...
  def initUI(self):
    ...
    self.nameVar = StringVar() #namevar 
    self.nameVar.set("SDH") #defualt text in namevar
    nameField = Entry(ipGroup, width=10, textvariable=self.nameVar)
    self.serverIPVar = StringVar() 
    self.serverIPVar.set("127.0.0.1") #serverIPvar default text
    serverIPField = Entry(ipGroup, width=15, textvariable=self.serverIPVar)
    self.serverPortVar = StringVar()
    self.serverPortVar.set("8090") # serverportvar default text
    serverPortField = Entry(ipGroup, width=5, textvariable=self.serverPortVar)
    serverSetButton = Button(ipGroup, text="Set", width=10, command=self.handleSetServer) #button for Set
    ...
    self.clientPortVar.set("8091") #clientPortvar
    ...
    self.receivedChats = Text(readChatGroup, bg="white", width=60, height=20, state=DISABLED)
    ...
    clearChatButton= Button(writeChatGroup, text ="Clear", width=10,command=self.handleClearChat)
    self.chatField.grid(row=0, column=1, sticky=W)
    sendChatButton.grid(row=2, column=1, padx=5)
    clearChatButton.grid(row=3, column=1, padx=5)

    self.statusLabel = Label(parentFrame)

    bottomLabel = Label(parentFrame, text="p2p chat. Adapted by siddharthasahu.in ")
    self.timeLabel = Label(parentFrame,text = date.today())
  ...
  def handleSetServer(self):
    if self.serverSoc != None:
        ...
        self.serverSoc = None
        self.serverStatus = 0
    serveraddr = (self.serverIPVar.get().replace(' ',''), int(self.serverPortVar.get().replace(' ',''))) #removing spaces
    try:
